...
<body>
  ...
  <div class="section-wrap">
    ...
    <div style="background: transparent; border: 2px solid #622632; border-radius:10px; padding:14px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
      <div>
        <div style="font-size:8px; font-weight:700; color:#8B1F2F; letter-spacing:1px; text-transform:uppercase; margin-bottom:4px;">Ganancia neta del período</div>
        <div style="font-size:26px; font-weight:700; color:#1A0A0A;">${fmt(data.resumen.ganancia)}</div>
        <div style="font-size:9px; color:#1A0A0A; margin-top:2px;">Lo que realmente te quedó</div>
      </div>
      <div style="text-align:right; display:flex; gap:20px;">
        <div>
          <div style="font-size:8px; color:#1A0A0A; margin-bottom:3px;">Ingresos brutos</div>
          <div style="font-size:13px; font-weight:600; color:#1A0A0A;">${fmt(data.resumen.ingresos_ventas)}</div>
        </div>
        <div>
          <div style="font-size:8px; color:#1A0A0A; margin-bottom:3px;">Gastos</div>
          <div style="font-size:13px; font-weight:600; color:#1A0A0A;">${fmt(data.resumen.total_gastos)}</div>
        </div>
        <div>
          <div style="font-size:8px; color:#1A0A0A; margin-bottom:3px;">Costo mercadería</div>
          <div style="font-size:13px; font-weight:600; color:#1A0A0A;">${fmt(data.resumen.cogs)}</div>
        </div>
      </div>
    </div>
  </div>
