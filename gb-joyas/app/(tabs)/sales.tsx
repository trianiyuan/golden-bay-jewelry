...
export default function SalesScreen() {
  ...
  function abrirEditar(venta: Venta) {
    ...
    reset({
      ...
      notas: venta.notas || '',
      total_recibido: String(venta.total_recibido),
    });
    setModalEditar(true);
  }
  ...
  function confirmarEliminar(venta: Venta) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <PageHeader title="Ventas" rightElement={<TouchableOpacity style={styles.btnPrimary} onPress={() => router.push('/sale/new')} activeOpacity={0.85}>
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            <View style={styles.canalesGrid}>
              {canales.map(canal => (
                <TouchableOpacity
                  key={canal.id}
                  style={[styles.canalChip, canalEditando === canal.id && styles.canalChipActive]}
                  onPress={() => setCanalEditando(canal.id)}
                >
                  <Text style={[styles.canalChipText, canalEditando === canal.id && styles.canalChipTextActive]}>
                    {canal.nombre}
                  </Text>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  btnPrimary: { backgroundColor: COLORS.wine, borderRadius: SIZES.radiusSm, paddingVertical: 8, paddingHorizontal: 13 },  btnPrimaryText: { fontSize: 12, fontWeight: '600', color: COLORS.surface },
  ...
  ventaCard: { backgroundColor: COLORS.surface, borderRadius: SIZES.radiusMd, borderWidth: 1, borderColor: COLORS.border, marginBottom: 10, overflow: 'hidden' },
  ventaMain: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12 },
  ...
  ventaAcciones: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: COLORS.border },
  ...
  entregaOpt: { flex: 1, borderRadius: SIZES.radiusMd, padding: 12, alignItems: 'center', borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface, gap: 4 },
